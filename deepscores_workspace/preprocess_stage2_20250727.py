#!/usr/bin/env python3
"""
Stage 2 DeepScores 데이터 전처리 스크립트
작성일: 2025-07-27
목적: stem, ledgerLine 등을 제외하고 핵심 기호만 학습하도록 데이터셋 재구성
"""
...
class Stage2Preprocessor:
    def __init__(self, config_file='stage2_preprocess_config.json'):
        # Stage 1 config를 기반으로 수정
        if not Path(config_file).exists():
            # Stage 1 config를 복사하여 수정
            with open('stage1_preprocess_config.json', 'r') as f:
                self.config = json.load(f)
            self.config['target_path'] = './data_stage2_20250727'
        ...
        # 제외할 클래스 ID (cat_id 기준)
        self.excluded_classes = {
            '42',   # stem
            '2',    # ledgerLine
            '6',    # beam (선택적 - notehead 연결로 재구성 가능)
            '68',   # tie (선택적 - notehead 간 곡선으로 재구성 가능)
            '54'    # slur (선택적 - 위치 기반 재구성 가능)
        }
    ...
    def regenerate_class_mapping(self):
        ...
        self.log(f"Selected classes: {len(self.selected_classes)}, Excluded: {len(self.excluded_classes)}")
    ...
    def create_yaml_config(self):
        ...
        yaml_path = self.target_path / 'deepscores_stage2.yaml'
    ...
    def run(self):
        """전체 전처리 실행"""
        self.log("🚀 Stage 2 Data Preprocessing Start")
